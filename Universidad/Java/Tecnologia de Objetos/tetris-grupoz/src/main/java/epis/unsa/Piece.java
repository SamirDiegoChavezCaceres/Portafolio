...
public final class Piece {
   ...
   */
  public Piece computeNextRotation() {                    //                              x 0 2
    // YOUR CODE HERE                                   //                        (-1,1)x x 0 1  *
    TPoint[] rotatedPoints = new TPoint[body.length]; //(x,y) = (0,0)(1,0)(1,1)(2,0)      x(0,0) * *
    for(int i = 0; i < body.length; i++){             //(-y,x) = (0,0)(0,1)(-1,1)(0,2)
      int x = (height - 1) - body[i].y;
      int y = body[i].x;
      rotatedPoints[i] = new TPoint(x,y);
    }
    return new Piece(rotatedPoints); // YOUR CODE HERE
  }
